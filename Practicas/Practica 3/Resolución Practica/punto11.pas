...
begin
  ...
  else if (tipodezona = 2) then begin 
...
var
  ...
  max: real;
begin 
  min := 999999999;  // Inicialización de min con un valor muy alto
  max := -1;  // Inicialización de max con un valor muy bajo
  contador := 0;
  recaudacion := 0;
  precio := 320;  // Precio de la soja
  repeat
    ...
    // Convertir localidad a minúsculas para comparar sin importar mayúsculas/minúsculas
    if (LowerCase(localidad) = 'tres de febrero') and (recaudacion > 10000) then begin
      contador := contador + 1;
    end;
    
    localidad_mayor_y_menor(recaudacion, localidad, localidad_menor, localidad_mayor, max, min);
    recaudacion := 0;  // Reiniciar recaudación para el siguiente campo
  until (cant_hectareas = 900) and (LowerCase(localidad) = 'saladillo'); 

  writeln;
  writeln('Cantidad de campos de la localidad de Tres de Febrero con un rendimiento estimado superior a U$S 10.000: ', contador);
  writeln('La localidad del campo con mayor rendimiento económico esperado: ', localidad_mayor);
  writeln('La localidad del campo con menor rendimiento económico esperado: ', localidad_menor);
end.
