...
begin
  ...
  gala.masa:=random(35)+1;
  gala.distancia:=random(59)+1;
end; 
...
begin
  if (v2[i].nombre ='sagitario') or (v2[i].nombre ='acuario') or (v2[i].nombre ='leo') then begin
...
begin
  if (v2[i].tipo = 4 ) and (v2[i].distancia < 1000 )then begin
    punto_c:=1;
  end
  else begin
    punto_c:=0;
...
begin
  if(v2[i].masa > max1)then begin
    ...
    if(v2[i].masa > max2)then begin  
      max2:=v2[i].masa;
      nom2_max:=v2[i].nombre;
    end;
  end;
  
  if(v2[i].masa < min1)then begin   
    min2:=min1;
    nom2_min:=nom1_min;
    min1:=v2[i].masa;
...
begin
  ...
  writeln('masa total de las 3 galaxias (sagiratio, acuario y leo) :',masa_totales:0:2,' y porcentaje: ',((masa_3_galaxias*masa_totales)/100):0:2);
  writeln('cantidad de galaxias no irregulares a menos de 1000 pc: ',cont_no_regulares);
  writeln('las dos galaxias con mayor masa: ',nom1_max,' y ',nom2_max,' y las dos galaxias con menor masa: ',nom1_min,' y ',nom2_min);
end;  
