...
begin
  ...
  while (l <> nil) do begin
    ...
    while (l <> nil) and (aux_cod_camion = l^.elem.cod_cam) do begin
      contador_km := contador_km + l^.elem.distancia_recorrida_km;
      if (impares(l^.elem.dni)) then begin
        writeln('Código de viaje con DNI impar: ', l^.elem.cod_viaje);
      end;
      contador_viajes := contador_viajes + condiciones(vec[l^.elem.cod_cam], l^.elem.anio);
      l := l^.sig;
    end;
    maximos_minimos(vec[l^.elem.cod_cam].patente, max_km, min_km, patente_min, patente_max, contador_km);
  end;

  writeln('Patente con más kilómetros recorridos: ', patente_max);
  writeln('Patente con menos kilómetros recorridos: ', patente_min);
  writeln('Cantidad de viajes en camiones con capacidad mayor a 30.5 toneladas y antigüedad mayor a 5 años: ', contador_viajes);
end;
